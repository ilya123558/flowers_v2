...
const socialList = [
  {title: 'Whatsapp', value: 'value-1'},
  {title: 'Telegram', value: 'value-2'},
  {title: 'Instagram', value: 'value-3'},
]
...
export const ProfilePersonalData = () => {
  ...
  const handleChangeSocial = (index: number, selectList: ISelectItem) => {
    const result = data.map((item, i) => index === i ? {...item, socialTitle: selectList.title} : item)
    setData(result)
  }
  ...
  return (
    <div className="mt-[16px] w-full p-[24px] rounded-[8px] bg-[#212121] roboto">
      <p className="leading-[151%] text-[16px] font-medium text-[#D9D9D9]">Как с вами связаться</p>
      <p className="leading-[151%] text-[16px] text-[#8C8C8C] mt-[8px]">
      ...
      <div className="flex flex-col gap-[24px] mt-[24px]">
        {data.map((item, index, arr) => (
          <div style={{zIndex: arr.length - index}} key={index} className="flex justify-between items-end gap-[16px] relative">
            <div className="flex gap-[16px] w-full">
              <InputSelect
                activeItem={socialList.find(socialItem => socialItem.title === item.socialTitle) || socialList[0]}
                selectList={socialList}
            ...
            <Button onClick={() => handleDeleteItem(index)} className="aspect-square h-[48px] flex-center bg-[#3A3A3A] rounded-[10px] active:scale-95">
        ...
        <div className="mt-[15px] flex gap-[16px]">
          {socialList.map((item, index) => (
            <div key={index} className="flex gap-[8px] items-center">
              <Button onClick={() => setActiveSocialItem(item.title)} className="border-[1px] border-white rounded-full aspect-square w-[17px] bg-[#4B4B4B] flex-center active:scale-95">
                <div className={`bg-[#303030] rounded-full aspect-square w-[8px] ${activeSocialItem === item.title && "!bg-yellow"}`}></div>
              </Button>
              <p className="text-[14px] text-[#FFFFFFD9]">{item.title}</p>
            </div>
          ))}
          <div className="flex gap-[8px] items-center">
            <Button onClick={() => setActiveSocialItem("Телефон")} className="border-[1px] border-white rounded-full aspect-square w-[17px] bg-[#4B4B4B] flex-center active:scale-95">
              <div className={`bg-[#303030] rounded-full aspect-square w-[8px] ${activeSocialItem === "Телефон" && "!bg-yellow"}`}></div>
            </Button>
