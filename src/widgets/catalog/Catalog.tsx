...
export const Catalog = ({className, isHidden = false}: TProps) => {
  ...
  return (
    <section className={clsx("pt-[43px] relative", className, isHidden && showAllProducts && "mb-[200px]")}>
      ...
      <Container className="">
        <div className="flex items-center justify-between mb-[27px]">
          ...
          <div className="flex items-center gap-[25px]">
            ...
            <Button onClick={() => setIsOpenfilter(!isOpenfilter)} className="aspect-square pl-[8px] active:scale-95">
              ...
              <path d="M19.707 10.9141L10.207 1.41406L0.707031 10.9141" stroke="#ADADAD" stroke-width="2"/>
              </svg>
            </Button>
          </div>
        </div>
        <AnimationHeightWrapper isOpen={isOpenfilter} className="">
          <div className="flex pt-[5px] flex-col gap-[32px]">
        ...
        <AnimationHeightWrapper isOpen={!isOpenfilter}>
          <div className="flex items-center gap-[20px] m-[15px_0px] pt-[5px]">
            <Title23px>{selectList.filter(item => item.isSelect).length === 0 ? "Ничего не выбрано" : "Выбрано"}</Title23px>
      ...
      <AnimationHeightWrapper isOpen maxHeight={showAllProducts ? 10000 : 1000} className="mt-[32px] relative">
        <Container className="">
          <ProductCatalog />
        </Container>
        <div className={clsx("bottom-0 w-screen absolute transition-all duration-500", showAllProducts && "pointer-events-none opacity-0")}>
          <div className="w-full relative h-[102px]">
            <Image src={'/images/catalog/hidden-bg.png'} alt="hidden-bg" fill />
