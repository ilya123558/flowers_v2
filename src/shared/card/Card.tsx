...
export const Card = (product: IProduct) => {
  ...
  return (
    <div className='!h-auto w-full flex'>
      <BoxWrapper className='sm:rounded-[16px] rounded-[8.8px] overflow-hidden h-full w-full bg-[#2D2D2D]'>
        <div className="sm:aspect-[290/345] aspect-[181/189] w-full relative">
          <img onClick={() => router.push('/product')} src={product.imgSrc} alt='product-item' width={290} height={345} className="cursor-pointer object-cover object-center w-full h-full"/>
          <div className="absolute bg-[#2B2B2BB2] sm:p-[7px_8px] p-[4.5px_6.15px] top-[9px] left-[8px] sm:rounded-[9px] rounded-[4.4px] flex-center gap-[4px]">
            <div className="relative sm:w-[16px] w-[8.8px] aspect-square">
              <Image src={'/images/product/coins.png'} alt='points' fill/>
            </div>
            <p className='text-[#DADADA] sm:text-[14px] text-[8px]'>{product.points} балла</p>
          </div>
          <div className="absolute top-[9px] right-[8px] rounded-[9px] flex sm:bg-[#2B2B2BB2]">
            <Button onClick={() => {}} className='active:scale-95'>
              <svg className="sm:w-[34px] sm:h-[34px] w-[24px] h-[24px]" width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="sm:p-[16px] p-[8.8px]">
          <div className='roboto'>
            <TextNowrap className='sm:text-[22px] text-[12px] font-black'>{product.title}</TextNowrap>
            <TextNowrap className='text-gray sm:text-[14px] text-[8px] sm:mt-[10px] mt-[5.5px]'>{product.category}</TextNowrap>
            <p className='sm:text-[20px] text-[11px] sm:mt-[15px] mt-[5.5px]'>{formatPrice(product.price)}</p>
            <ul className="flex sm:gap-[8px] gap-[4.4px] sm:h-[43px] h-[27px] sm:mt-[10px] mt-[5.5px]">
              {product.tags.slice(0, 2).map((tag, i) => (
                <li key={i} className="bg-[#252525] flex-center sm:p-[0px_10px] p-[0px_5.5px] sm:rounded-[8px] rounded-[4.4px]">
                  <p className="sm:text-[14px] text-[7.7px]">{tag}</p>
                </li>
              ))}

              {product.tags.length > 2 && (
                <li className="bg-[#252525] flex-center sm:p-[0px_10px] p-[0px_10px] sm:rounded-[8px] rounded-[4.4px]">
                  <p className="sm:text-[14px] text-[7.7px]">...</p>
                </li>
              )}
            </ul>
          </div>
          <div className="sm:mt-[16px] mt-[8.4px] flex items-center gap-[9px] sm:h-[42px] h-[23px]">
            <Button onClick={() => dispatch(incCount())} className='h-full w-full border-[1px] border-yellow sm:rounded-[11px] rounded-[6px] active:scale-95'>
              <div className="flex-center gap-[8px]">
                <svg className="sm:w-[18px] sm:h-[19px] w-[8.5px] h-[9.3px]" width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <p className='text-yellow sm:text-[16px] text-[8.8px] font-bold'>В корзину</p>
              </div>
            </Button>
            <Button onClick={() => router.push('/product')} className='h-full w-full rounded-[11px] active:scale-95'>
              <div className="flex-center gap-[8px] roboto-flex">
                <p className='text-yellow sm:text-[16px] text-[8.8px] font-bold'>Подробнее</p>
              </div>
            </Button>
          </div>
        </div>
